###################################################
# Ace Combat 6: Fires of Liberation - FHM scanner #
...
if len(file_list) != 0:
    ...
    Please use values of power of two (512, 1024, 2048, 4096, etc.) if possible.
    """
    ...
    output_log = open(basedir + "//fhm_sniffer_log.txt", "w")
    ...
    for i in range(len(file_list)):
            ...
            ouput_directory = basedir + "\\" + file_name[i]
            ...
            while not (chunk_total_read > dmp_file_size):
                    ...
                    # Keep track of the offset with the last processable FHM file.
                    ...
                    for match_obj in re.finditer(fhm_signature, chunk.read()):
                            ...
                            """
                            # Skip smaller FHM containers inside larger FHM containers.
                            
                            The "fhm_offset_skip" variable contains the offset of the last processable FHM with its file size added.
                            
                            If the current FHM file's position value is less than the one stored in "fhm_offset_skip" then it means
                            that said FHM file is possibly part of a larger FHM file. The current position (and subsequent ones) will 
                            be skipped until its value is larger that the one stored in the variable.
                            """
                            
                            if match_obj.start(0) > fhm_offset_skip :
                                ...
                                if not nof == 0:
                                        ...
                                        if not (fhm_filesize >= 268435456 or fhm_filesize <= 0): # If the FHM file's size is equal or smaller than zero or bigger than 256 mb then skip it.
                                                ...
                                                fhm_offset_skip = match_obj.start(0) + fhm_filesize
            ...
            print()
            output_log.write(">> Total amount of FHM containers found and dumped to [\\" + file_name[i] + "]: " + str(fhm_found_count) + "\n\n")
